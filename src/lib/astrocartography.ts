...
import type { Planet, LineType, AstroLine } from '../types/index.ts'
...
function getGST(date: Date): number {
  const astroDate = Astronomy.MakeTime(date)
  return Astronomy.SiderealTime(astroDate) * 15 // hours to degrees
}
...
export function getLineDistanceToPoint(
  ...
  // Scale longitude difference by cos(latitude) so 1 degree of longitude
  // at high latitudes isn't over-counted relative to 1 degree of latitude.
