import { useMemo, useState } from 'react'
import { AnimatePresence, motion } from 'framer-motion'
import { useStore } from '../store/useStore.ts'
import { PLANET_COLORS, PLANETS } from '../lib/astrocartography.ts'
import { ENERGY_TIERS, LINE_TYPE_STYLES } from '../lib/mapGuidance.ts'

export default function MapKey() {
  const enabledPlanets = useStore((s) => s.enabledPlanets)
  const [open, setOpen] = useState(false)

  const visiblePlanets = useMemo(
    () => PLANETS.filter((planet) => enabledPlanets.has(planet)),
    [enabledPlanets],
  )

  return (
    <div className="absolute bottom-6 right-6 z-20">
      <button
        onClick={() => setOpen((v) => !v)}
        className="h-10 rounded-xl bg-white/92 backdrop-blur-md border border-border/60 shadow-sm
                   px-3.5 flex items-center gap-2 text-xs font-semibold text-text cursor-pointer
                   hover:bg-white transition-colors min-h-[44px]"
        aria-expanded={open}
        aria-controls="map-key-panel"
      >
        <svg className="size-4 text-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.7}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M3 6h18M3 12h18M3 18h18" />
        </svg>
        Map Key
      </button>

      <AnimatePresence>
        {open && (
          <motion.aside
            id="map-key-panel"
            className="absolute bottom-12 right-0 w-[320px] rounded-2xl border border-border/70 bg-white/96
                       backdrop-blur-xl shadow-lg p-4 space-y-4"
            initial={{ opacity: 0, y: 8, scale: 0.98 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 6, scale: 0.98 }}
            transition={{ duration: 0.16 }}
          >
            <div>
              <p className="text-[11px] font-semibold tracking-wide uppercase text-muted mb-2">
                Dot Color = Energy
              </p>
              <div className="grid grid-cols-2 gap-x-3 gap-y-2">
                {ENERGY_TIERS.map((tier) => (
                  <div key={tier.id} className="flex items-center gap-2">
                    <span
                      className="size-2.5 rounded-full border border-white/70 flex-shrink-0"
                      style={{ backgroundColor: tier.color }}
                    />
                    <span className="text-[11px] text-text tabular-nums">
                      {tier.label} <span className="text-muted">{tier.rangeLabel}</span>
                    </span>
                  </div>
                ))}
              </div>
            </div>

            <div>
              <p className="text-[11px] font-semibold tracking-wide uppercase text-muted mb-2">
                Line Color = Planet
              </p>
              <div className="grid grid-cols-2 gap-x-3 gap-y-2">
                {visiblePlanets.map((planet) => (
                  <div key={planet} className="flex items-center gap-2">
                    <span
                      className="size-2.5 rounded-full border border-white/70 flex-shrink-0"
                      style={{ backgroundColor: PLANET_COLORS[planet] }}
                    />
                    <span className="text-[11px] text-text">{planet}</span>
                  </div>
                ))}
              </div>
            </div>

            <div>
              <p className="text-[11px] font-semibold tracking-wide uppercase text-muted mb-2">
                Line Style = Angle
              </p>
              <div className="space-y-2">
                {LINE_TYPE_STYLES.map((style) => (
                  <div key={style.lineType} className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-[11px] font-medium text-text">
                        {style.lineType} · {style.label}
                      </p>
                      <p className="text-[10px] text-muted">{style.context}</p>
                    </div>
                    <svg className="flex-shrink-0" width="58" height="8" viewBox="0 0 58 8" aria-hidden="true">
                      <line
                        x1="0"
                        y1="4"
                        x2="58"
                        y2="4"
                        stroke="#4B5563"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeDasharray={style.legendDasharray}
                      />
                    </svg>
                  </div>
                ))}
              </div>
            </div>
          </motion.aside>
        )}
      </AnimatePresence>
    </div>
  )
}
